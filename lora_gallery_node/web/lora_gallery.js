...
app.registerExtension({
  name: "LoRAGalleryNode",

  async beforeRegisterNodeDef(nodeType, nodeData) {
    if (nodeData.name !== "LoRAGalleryNode") return;
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const btnRef  = document.createElement("button"); btnRef.className = "lgg-btn"; btnRef.textContent = "⟳";
      ...
      tb.append(btnML, btnAct, btnEdit, btnRef, btnRst, cntEl);
    ...
    nodeType.prototype._lggRenderBrowse = function () {
      ...
      const groups = {};
      for (const l of list) { const k = l.model_type || "Other"; (groups[k] = groups[k] || []).push(l); }
      const keys = Object.keys(groups).sort((a, b) => a === "Other" ? 1 : b === "Other" ? -1 : a.localeCompare(b));
      if (!keys.length) {
      ...
      for (const mt of keys) {
        const loras = groups[mt], open = this._lggGrpOpen[mt] !== false;
        const nSel  = loras.filter(l => this._lggSel[l.relative_path]).length;
        const grp   = document.createElement("div"); grp.className = "lgg-group";
        const hdr   = document.createElement("div"); hdr.className = "lgg-group-hdr" + (open ? " open" : "");
        const arr   = document.createElement("span"); arr.className = "arr"; arr.textContent = "▶";
        const lbl   = document.createElement("span"); lbl.textContent = mt;
        const badge = document.createElement("span"); badge.className = "lgg-group-badge" + (nSel ? " lit" : ""); badge.textContent = nSel ? `${nSel}/${loras.length}` : loras.length;
        hdr.append(arr, lbl, badge);
        const gbody = document.createElement("div"); gbody.className = "lgg-group-body" + (open ? " open" : "");
        hdr.addEventListener("click", () => { const n = !hdr.classList.contains("open"); hdr.classList.toggle("open", n); gbody.classList.toggle("open", n); this._lggGrpOpen[mt] = n; this._lggCommit(); });
        for (const l of loras) gbody.appendChild(this._lggCard(l, false, null, null));
        grp.append(hdr, gbody); body.appendChild(grp);
      }
    ...
    nodeType.prototype._lggToggle = function (lora, card, chk) {
      ...
      this._lggCommit(); this._lggUpdateCount(); this._lggUpdateStatus(); this._lggUpdateGrpBadge(lora.model_type); this._lggUpdateSecBadges();
    };
